...
#### ReviewQueryRepositoryTest
QueryDSL을 사용한 동적 쿼리 기능의 정확성을 검증하는 테스트를 작성했습니다.

**테스트 항목**:

1. **책 제목 검색 테스트** (`searchReviews_byBookTitle`)
   - "자바" 키워드로 검색했을 때 해당 키워드가 포함된 리뷰만 반환되는지 확인
   - QueryDSL의 동적 쿼리 생성 기능 검증
...
2. **평점 범위 검색 테스트** (`searchReviews_byRatingRange`)
   - 평점 4~5점 범위의 리뷰만 올바르게 필터링되는지 확인
   - 최소/최대 평점 조건이 정확히 적용되는지 검증
...
3. **사용자ID 검색 테스트** (`searchReviews_byUserId`)
   - 특정 사용자가 작성한 리뷰만 조회되는지 확인
   - 사용자별 리뷰 필터링 기능 검증
...
4. **복합 조건 검색 테스트** (`searchReviews_multipleConditions`)
   - 책 제목, 최소 평점, 사용자ID를 동시에 적용한 복합 검색 테스트
   - 여러 조건이 AND 연산으로 정확히 결합되는지 확인
...
5. **페이징 처리 테스트** (`searchReviews_withPaging`)
   - 페이지 크기, 전체 요소 수, 전체 페이지 수가 올바른지 확인
   - Spring Data의 Page 객체가 정확히 생성되는지 검증
...
6. **정렬 처리 테스트** (`searchReviews_withSorting`)
   - 평점 내림차순 정렬이 제대로 작동하는지 확인
   - OrderSpecifier를 통한 동적 정렬 기능 검증
...
**테스트 전략**:
- `@DataJpaTest`를 사용하여 실제 H2 데이터베이스와 통합 테스트
- `/data.sql`을 통해 테스트 데이터를 미리 준비 (15개의 리뷰 데이터)
- AssertJ를 활용한 명확한 검증 코드 작성
- 각 테스트는 독립적으로 실행 가능하도록 설계
- given-when-then 패턴으로 테스트 가독성 향상

**테스트 결과**:
- 총 6개의 테스트 케이스 작성
- 동적 쿼리, 페이징, 정렬 등 QueryDSL의 핵심 기능 모두 검증
- 모든 테스트 케이스 통과 확인
...
### 향후 개선 계획
1. **테스트 코드 강화**
   - 엣지 케이스(빈 결과, null 값 등) 테스트 추가하기
   - 성능 테스트 추가 (대용량 데이터 페이징)
