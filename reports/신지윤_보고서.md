...
우리가 만든 프로젝트는 책 리뷰를 관리하는 API 서버입니다. 사람들이 로그인하고 리뷰를 쓰고, 수정하고, 삭제할 수 있습니다. 특히 입력 데이터 검증을 엄격하게 해서 이상한 데이터가 들어가지 않도록 만들었습니다.
...
**ReviewService**에서는 실제 로직을 처리했습니다. 다른 사람이 내 리뷰를 수정하거나 삭제하지 못하도록 소유권 검사도 넣었고요.
...
**어떻게 해결했는지**:
처음에는 왕색초보라서 딈는 것도 모르고 있었는데, Spring Boot에 validation 라이브러리를 추가해야 한다는 걸 알았어요:
```gradle
...
**결과**: 이제 잘못된 데이터는 미리 차단되고 사용자에게 친절한 오류 메시지가 간다
